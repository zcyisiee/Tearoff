...
enum FileStorage {
    ...
    /// Hidden sidecar directory name. EdgeMark vaults used `.edgemark/`; those are
    /// renamed to `.tearoff/` on first access.
    static let sidecarDirectoryName = ".tearoff"
    static let legacySidecarDirectoryName = ".edgemark"

    /// Sidecar directory at `root`, migrating `.edgemark/` → `.tearoff/` when needed.
    static func sidecarDirectoryURL(in root: URL) -> URL {
        let fm = FileManager.default
        let current = root.appendingPathComponent(sidecarDirectoryName, isDirectory: true)
        let legacy = root.appendingPathComponent(legacySidecarDirectoryName, isDirectory: true)
        let currentMeta = current.appendingPathComponent("meta.json")
        let legacyMeta = legacy.appendingPathComponent("meta.json")

        if fm.fileExists(atPath: currentMeta.path) {
            return current
        }
        if fm.fileExists(atPath: legacyMeta.path) {
            ...
            if !fm.fileExists(atPath: current.path) {
                do {
                    ...
                    Log.storage.info("[Sidecar] migrated .edgemark → .tearoff")
                    return current
                } catch {
                    Log.storage.error("[Sidecar] failed to rename .edgemark → .tearoff: \(error)")
                    return legacy
                }
            }
            return legacy
    ...
    /// `.tearoff/` (sidecar) and `.trash/` subdirs. Used when adding a new location
    ...
    static func ensureRootStructure(at url: URL) throws {
        ...
        try fm.createDirectory(at: sidecarDirectoryURL(in: url), withIntermediateDirectories: true)
