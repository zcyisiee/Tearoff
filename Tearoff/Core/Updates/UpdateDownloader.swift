...
final nonisolated class UpdateDownloader: NSObject, Sendable, URLSessionDownloadDelegate {
    ...
    static func download(
        ...
        let tempDir = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first!
            .appendingPathComponent("Tearoff")
        ...
        let targetDir = tempDir.appendingPathComponent("TearoffUpdate-\(UUID().uuidString)")
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            config.httpAdditionalHeaders = ["User-Agent": "Tearoff/\(version) (macOS)"]
    ...
    nonisolated func urlSession(
        ...
        let destination = targetDirectory.appendingPathComponent("Tearoff-update.dmg")
        do {
